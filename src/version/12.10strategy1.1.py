...
class BatchScheduler:
    ...
    def _detect_all_elevator_conflicts(self, assignments: Dict, current_time: float, sim_elevators: Dict) -> Dict[str, List]:
        """
        检测所有电梯的所有冲突
    ...
    def _resolve_conflicts_priority(self, all_conflicts: Dict, assignments: Dict,
        ...
        for elevator_id, conflict_groups in all_conflicts.items():
            for conflict_group in conflict_groups:
                ...
                # 改进的排序：先按持续时间（从长到短），再按任务ID
                sorted_conflicts = sorted(conflict_group,
                                          key=lambda x: (-x['duration'], x['task_id']))  # 负号表示降序

                # 第一个任务（持续时间最长）获得电梯使用权
                winner = sorted_conflicts[0]
                winners = [winner]
                losers = sorted_conflicts[1:]

                print(f"  获胜任务（长任务优先）: 任务{winner['task_id']}({winner['duration']:.1f}s)")
    ...
    def _force_resolve_conflicts(self, all_conflicts: Dict, assignments: Dict, all_path_options: Dict):
        ...
        for elevator_id, conflict_groups in all_conflicts.items():
            for conflict_group in conflict_groups:
                # 按任务持续时间排序（从短到长），短任务强制切换到楼梯
                sorted_conflicts = sorted(conflict_group, key=lambda x: x['duration'])

                for i, conflict in enumerate(sorted_conflicts):
                    if i >= 1:  # 只保留一个任务使用电梯（最长任务），其他强制切换
            ...
            if best_alt is not None:
                ...
                if elevator_id:
                    ...
                    if elev.schedule:  # 如果电梯已经有任务
                        ...
                        # 排序：首先按任务开始时间排序，然后按任务耗时排序
                        scheduled_tasks.sort(key=lambda t: (t[0], t[1] - t[0]))  # 优先级：先开始的，后开始的耗时短的优先
