...
class Stair:
    ...
    def __init__(self, stair_id: str, building_num: int, floor_num: int):
        ...
        self.usage_schedule = []  # (start_time, end_time, robot_id, task_id)
...
class Robot:
    def __init__(self, rid: int, skill: str, position: str):
        ...
        self.wait_time = 0.0
        self.stair_wait_time = 0.0  # 新增：楼梯等待时间
    ...
    def copy(self):
        ...
        new_robot.stair_wait_time = self.stair_wait_time
...
# ============================================================
# 批量调度器 - 核心新增组件（增强版：包含楼梯冲突检测）
# ============================================================
class BatchScheduler:
    """
    批量任务调度器，重点解决电梯冲突和路径重选问题，新增楼梯冲突检测
    """
    ...
    def __init__(self, robots, elevators, stair_graph, elevator_graphs):
        ...
        self.stairs = self._init_stairs()  # 新增：初始化楼梯
        self.stair_wait_records = {}  # 新增：记录楼梯等待信息

    def _init_stairs(self):
        """初始化楼梯对象"""
        stairs = {}
        ...
        for building in [1, 2, 3]:
            max_floors = 9 if building == 2 else 6 if building in [1, 3] else 0
            for floor in range(1, max_floors + 1):
                # 楼梯1
                stairs[f"Stair1_b{building}_f{floor}"] = Stair(f"Stair1_b{building}_f{floor}", building, floor)
                # 楼梯2
                stairs[f"Stair2_b{building}_f{floor}"] = Stair(f"Stair2_b{building}_f{floor}", building, floor)
    ...
    def batch_assign_tasks(self, tasks: List[Task], current_time: float) -> Dict:
        """
        批量分配任务的核心算法
        支持电梯冲突检测、楼梯冲突检测和路径重选，并记录最后完成的任务时间
        """
        ...
        # 步骤3: 新增：检测并解决楼梯冲突
        print("\n=== 开始楼梯冲突检测 ===")
        final_assignments = self._detect_and_resolve_stair_conflicts(final_assignments, current_time)

        # 步骤4: 执行最终分配，并记录任务完成时间
        ...
        # 计算最后完成的任务的完成时间
        latest_completion_time = max(task['path_info']['actual_time'] for task in final_assignments.values())
        print(f"最后完成的任务的完成时间: {latest_completion_time:.2f}s")

        # 新增：输出楼梯等待信息
        self._print_stair_wait_summary()
    ...
    def _get_best_available_path_for_task(
        ...
        current_time: float,
        sim_elevators: Dict,
        max_retries: int = 5  # 默认最多尝试5次
    ):
        """
        为任务选择最优路径，如果次优路径与其他机器人冲突，则尝试第三优路径，以此类推。
        如果找到了一个没有冲突的路径，则返回该路径，否则返回 None。
        """
        ...
        current_info = current_assignment['path_info']

        # 当前方案耗时
        current_time_cost = current_info['actual_time']

        task_options = all_path_options.get(task_id, [])
        best_alt = None
        best_alt_time = float('inf')
        ...
        while retry_count < max_retries:
            ...
            for option in task_options:
                opt_robot = option['robot']
                path_type = option['path_type']
                path_info = option['path_info']

                # 只考虑同一个机器人的不同路径类型，
                if opt_robot.id != robot.id:
                    continue
                if path_type == current_path_type:
                    continue

                # 计算替代方案的实际耗时：电梯路径要考虑等待，楼梯直接用 actual_time
                if path_info['type'] == 'elevator':
                    alt_time = self._calculate_actual_elevator_time(
                        path_info, robot.id, current_time, sim_elevators
                    )
                else:
                    alt_time = path_info['actual_time']

                if alt_time < best_alt_time:
                    best_alt_time = alt_time
                    best_alt = {
                        "new_path_type": path_type,
                        "new_path_info": path_info.copy(),
                        "delta": alt_time - current_time_cost
                    }

            if best_alt is not None:
                # 检查该次优路径是否与其他任务发生冲突
                elevator_id = best_alt["new_path_info"].get('eid')
                if elevator_id:
                    elev = sim_elevators[elevator_id]
                    if elev.schedule:  # 如果电梯已经有任务
                        # 获取所有在同一电梯上排队的任务（包括当前任务）
                        scheduled_tasks = [t for t in elev.schedule if t[4] == robot.id]

                        # 排序：首先按任务持续时间排序（长任务优先），然后按任务开始时间排序
                        scheduled_tasks.sort(key=lambda t: (-(t[1] - t[0]), t[0]))  # 优先级：先排序耗时更长的，再排序先开始的

                        # 检查次优路径是否会与更高优先级的任务冲突
                        for scheduled_task in scheduled_tasks:
                            scheduled_start = scheduled_task[0]
                            scheduled_end = scheduled_task[1]
                            if best_alt_time > scheduled_end:  # 次优路径的结束时间与已有任务不冲突
                                break
                            else:
                                best_alt = None  # 发生冲突，换下一个次优路径
                                break

                if best_alt is not None:
                    # 如果没有冲突，返回这个次优路径
                    best_alt["new_path_info"]["actual_time"] = best_alt_time
                    return best_alt

            return None  # 超过最大尝试次数仍然找不到合适的路径时，返回 None

    def _resolve_conflicts_prioritize_long_elevator(
        self,
        all_conflicts: Dict,
        assignments: Dict,
        all_path_options: Dict,
        ...
        """
        使用"优先让使用电梯时间最长的任务使用电梯"的策略解决电梯冲突：
        - 对每一组电梯冲突任务，按电梯使用时间（between）从长到短排序
        - 最长的任务保留电梯路径，其他任务尝试切换到次优路径
        - 更新任务的 assignment 后，返回 True 表示有修改
        """
        updated = False
            ...
            for group in conflict_groups:
                ...
                for req in group:
                    ...
                    if task_id in assignments:
                        ...
                        if path_info['type'] == 'elevator' and path_info['eid'] == elevator_id:
                            conflict_tasks_info.append({
                                'task_id': task_id,
                                'assignment': assignment,
                                'elevator_time': path_info['between'],  # 电梯使用时间
                                'robot_id': assignment['robot'].id
                            })

                if len(conflict_tasks_info) <= 1:
                    continue

                # 按电梯使用时间从长到短排序
                conflict_tasks_info.sort(key=lambda x: x['elevator_time'], reverse=True)
                task_ids = [info['task_id'] for info in conflict_tasks_info]
                print(f"电梯 {elevator_id} 冲突任务组: {task_ids}")
                
                time_info = [(info['task_id'], f"{info['elevator_time']:.2f}s") for info in conflict_tasks_info]
                print(f"  电梯使用时间排序: {time_info}")

                # 最长的任务保留电梯路径
                longest_task = conflict_tasks_info[0]
                print(f"  任务{longest_task['task_id']}保留电梯路径（使用时间: {longest_task['elevator_time']:.2f}s）")

                # 其他任务尝试切换到次优路径
                for i in range(1, len(conflict_tasks_info)):
                    task_info = conflict_tasks_info[i]
                    task_id = task_info['task_id']
                    
                    # 寻找次优路径
                    alt = self._get_best_available_path_for_task(
                        task_id,
                        assignments,
                        all_path_options,
                        current_time,
                    ...
                    if alt is not None:
                        old_assignment = assignments[task_id]
                        ...
                        print(
                            f"  任务 {task_id} 切换为 {new_type}, "
                            f"时间: {old_time:.2f}s -> {new_info['actual_time']:.2f}s "
                            f"(Δ = {alt['delta']:+.2f}s)"
                        )
                        updated = True
                    else:
                        print(f"  任务 {task_id} 没有可用的次优路径，保持原路径")
    ...
    def _resolve_conflicts_and_assign(self, all_path_options: Dict, tasks: List[Task], current_time: float) -> Dict:
        """
        改进的冲突解决算法：支持多任务同时冲突，使用长电梯任务优先策略
        """
        print("开始冲突解决和路径重选...")
        ...
        for iteration in range(max_iterations):
            ...
            # 解决冲突：优先让使用电梯时间最长的任务使用电梯
            updated = self._resolve_conflicts_prioritize_long_elevator(all_conflicts, current_assignments,
    ...
    # 新增：楼梯冲突检测模块
    # ============================================================
    def _detect_and_resolve_stair_conflicts(self, assignments: Dict, current_time: float) -> Dict:
        """
        检测并解决楼梯冲突
        策略：剩余任务时间最短的机器人先走楼梯，其他冲突的机器人在楼梯口等着
        """
        print("开始楼梯冲突检测...")
        
        # 重置楼梯等待记录
        self.stair_wait_records = {}
        ...
        for task_id, assignment in assignments.items():
            ...
            if path_info['type'] == 'stair':
                # 识别路径中的楼梯段
                stair_segments = self._identify_stair_segments_in_path(path_info['path'])
                if stair_segments:
                    stair_using_tasks.append({
                        'task_id': task_id,
                        'robot': assignment['robot'],
                        'stair_segments': stair_segments,
                        'total_task_time': path_info['actual_time'],
                        'assignment': assignment
                    })
        
        if not stair_using_tasks:
            print("✅ 无任务使用楼梯")
            return assignments
        
        print(f"发现 {len(stair_using_tasks)} 个任务使用楼梯")
        
        # 按楼梯分组
        stair_groups = defaultdict(list)
        for task_info in stair_using_tasks:
            for stair_segment in task_info['stair_segments']:
                stair_id = stair_segment['stair_id']
                stair_groups[stair_id].append({
                    'task_id': task_info['task_id'],
                    'robot': task_info['robot'],
                    'stair_segment': stair_segment,
                    'total_task_time': task_info['total_task_time'],
                    'assignment': task_info['assignment']
                })
        
        # 为每个楼梯解决冲突
        for stair_id, task_infos in stair_groups.items():
            if len(task_infos) <= 1:
                # 单个任务使用，无冲突
                continue
                
            print(f"\n检测楼梯 {stair_id} 冲突: {len(task_infos)} 个任务")
            
            # 按任务总时间排序（时间短的优先）
            sorted_tasks = sorted(task_infos, key=lambda x: x['total_task_time'])
            
            # 选择优先机器人（任务时间最短的）
            priority_task = sorted_tasks[0]
            other_tasks = sorted_tasks[1:]
            
            # 修复：避免嵌套的f-string引用问题
            waiting_robots = []
            for t in other_tasks:
                waiting_robots.append(f'R{t["robot"].id}')
            
            print(f"  优先机器人: R{priority_task['robot'].id} (任务时间: {priority_task['total_task_time']:.2f}s)")
            print(f"  其他机器人: {waiting_robots}")
            
            # 记录等待信息
            self.stair_wait_records[stair_id] = {
                'priority_robot': priority_task['robot'].id,
                'priority_task_time': priority_task['total_task_time'],
                'waiting_robots': [t['robot'].id for t in other_tasks],
                'wait_times': {},
                'total_wait_time': 0.0
            }
            
            # 优先机器人无等待
            stair_duration = priority_task['stair_segment']['duration']
            priority_start_time = current_time
            
            # 其他机器人需要等待
            last_end_time = priority_start_time + stair_duration
            total_wait_time_for_stair = 0.0
            
            for other_task in other_tasks:
                # 计算等待时间
                wait_time = max(0, last_end_time - current_time)
                
                # 记录等待时间
                self.stair_wait_records[stair_id]['wait_times'][other_task['robot'].id] = wait_time
                total_wait_time_for_stair += wait_time
                
                # 更新其他机器人的路径时间（增加等待时间）
                other_assignment = assignments[other_task['task_id']]
                original_time = other_assignment['path_info']['actual_time']
                new_time = original_time + wait_time
                other_assignment['path_info']['actual_time'] = new_time
                other_assignment['path_info']['stair_wait_time'] = wait_time
                
                print(f"  机器人 R{other_task['robot'].id} 等待楼梯 {wait_time:.2f}s, "
                      f"时间: {original_time:.2f}s -> {new_time:.2f}s")
                
                # 更新机器人对象的等待时间
                other_task['robot'].stair_wait_time = wait_time
                
                # 更新楼梯使用结束时间
                last_end_time = current_time + wait_time + other_task['stair_segment']['duration']
            
            self.stair_wait_records[stair_id]['total_wait_time'] = total_wait_time_for_stair
        
        print("✅ 楼梯冲突解决完成")
        return assignments
    
    def _identify_stair_segments_in_path(self, path: List[str]) -> List[Dict]:
        """
        识别路径中的楼梯段
        返回: [{"stair_id": "Stair1_1", "entry_node": "1_1_Stair1_1", "duration": 10.0}, ...]
        """
        stair_segments = []
        ...
        for i in range(len(path) - 1):
            ...
            # 检查是否是楼梯节点
            if "Stair1_1" in node1 and "Stair1_1" in node2 and node1 != node2:
                stair_segments.append({
                    'stair_id': 'Stair1_1',
                    'entry_node': node1,
                    'duration': self._estimate_stair_duration(node1, node2)
                })
            elif "Stair1_2" in node1 and "Stair1_2" in node2 and node1 != node2:
                stair_segments.append({
                    'stair_id': 'Stair1_2',
                    'entry_node': node1,
                    'duration': self._estimate_stair_duration(node1, node2)
                })
            elif "Stair2_1" in node1 and "Stair2_1" in node2 and node1 != node2:
                stair_segments.append({
                    'stair_id': 'Stair2_1',
                    'entry_node': node1,
                    'duration': self._estimate_stair_duration(node1, node2)
                })
            elif "Stair2_2" in node1 and "Stair2_2" in node2 and node1 != node2:
                stair_segments.append({
                    'stair_id': 'Stair2_2',
                    'entry_node': node1,
                    'duration': self._estimate_stair_duration(node1, node2)
    ...
    def _estimate_stair_duration(self, node1: str, node2: str) -> float:
        """
        估算楼梯段的持续时间
        这里需要根据你的实际图结构来计算
        """
        # 简单估算：楼层差 * 每层时间
        try:
            floor1 = int(node1.split('_')[0])
            floor2 = int(node2.split('_')[0])
            floor_diff = abs(floor1 - floor2)
            return floor_diff * 3.0  # 假设每层3秒
        except:
            return 5.0  # 默认值
    
    def _print_stair_wait_summary(self):
        """输出楼梯等待情况汇总"""
        if not self.stair_wait_records:
            print("\n=== 楼梯等待情况汇总 ===")
            print("无楼梯等待")
            return
        
        print("\n=== 楼梯等待情况汇总 ===")
        total_stair_wait_time = 0
        
        for stair_id, wait_info in self.stair_wait_records.items():
            if wait_info['waiting_robots']:
                print(f"楼梯 {stair_id}:")
                print(f"  优先机器人: R{wait_info['priority_robot']} (任务时间: {wait_info['priority_task_time']:.2f}s)")
                print(f"  等待机器人: {wait_info['waiting_robots']}")
                
                # 输出每个机器人的等待时间
                for robot_id, wait_time in wait_info['wait_times'].items():
                    print(f"    R{robot_id} 等待: {wait_time:.2f}s")
                
                print(f"  总等待时间: {wait_info['total_wait_time']:.2f}s")
                total_stair_wait_time += wait_info['total_wait_time']
        
        if total_stair_wait_time > 0:
            print(f"\n楼梯总等待时间: {total_stair_wait_time:.2f}s")
        else:
            print("无楼梯等待")
    ...
    def _execute_final_assignments(self, assignments: Dict, current_time: float) -> Dict:
        ...
        for task_id in sorted(assignments.keys()):
            ...
            robot.stair_wait_time = path_info.get('stair_wait_time', 0.0)
            ...
            current_paths[task_id] = {
                ...
                "stair_wait_time": path_info.get('stair_wait_time', 0.0)
            }
            ...
            results[task_id] = {
                ...
                "path_type": path_type,
                "total_time": path_info['actual_time'],
                "elevator_wait_time": path_info.get('wait_time', 0.0),
                "stair_wait_time": path_info.get('stair_wait_time', 0.0)
            }
...
def get_robot_status_real_time(current_timestamp=None):
    ...
    for r in scheduler.robots:
        ...
        if r.path and len(r.path) > 0:
            try:
                # 考虑总等待时间（电梯等待 + 楼梯等待）
                total_wait_time = r.wait_time + r.stair_wait_time
        ...
        data_list.append({
            ...
            "stair_wait_time": round(r.stair_wait_time, 2),  # 新增：楼梯等待时间
